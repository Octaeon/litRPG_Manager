...
impl Storage {
    ...
    pub fn createVariable(&mut self, variable: String, val: i32) -> Result<(), RuntimeErr> {
        ...
        // with that name.
        self.variables.insert(variable, val).map_or(Ok(()), |_| {
            Err((RuntimeErr::TriedToInitializeExistingVariable))
        })
    }
    ...
    pub fn modifyVariable<F: FnOnce(i32) -> i32>(
        ...
        match self.variables.get(&key) {
            ...
            None => Err(RuntimeErr::TriedToModifyNonexistentVariable),
        }
    }
}
